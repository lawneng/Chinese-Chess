...
function onClick(event) {
    ...
    if (checkValidMovement(x, y) === true) {
        if (checkValidCapture(x, y) === true) {
            if (board.tiles[y][x].pieceType === CANNON) {
...
function checkPossiblePlays() {
    ...
    else if (tile.pieceType === CAR) checkPossiblePlaysCAR(curX, curY);
    else if (tile.pieceType === HORSE) checkPossiblePlaysHORSE(curX, curY);
    else if (tile.pieceType === ELEPHANT) checkPossiblePlaysELEPHANT(curX, curY);
    else if (tile.pieceType === GUARD) checkPossiblePlaysGUARD(curX, curY);
    else if (tile.pieceType === CANNON) checkPossiblePlaysCANNON(curX, curY);
}
...
function checkPossiblePlaysCAR(curX, curY) {
    // Far left moves
    if (curX-2 >= 0) {
        // Upper move
...
function checkPossiblePlaysELEPHANT(curX, curY) {
    // Upper move
...
function checkPossiblePlaysGUARD(curX, curY) {
    checkPossiblePlaysHORSE(curX, curY);
    checkPossiblePlaysELEPHANT(curX, curY);
}

function checkPossiblePlaysCANNON(curX, curY) {
    for (let i = -1; i <= 1; i++) {
...
class Board {
    constructor() {
        ...
        this.tiles.push([
            new Tile(ELEPHANT, BLACK),
            new Tile(CAR, BLACK),
            new Tile(HORSE, BLACK),
            new Tile(GUARD, BLACK),
            new Tile(CANNON, BLACK),
            new Tile(HORSE, BLACK),
            new Tile(CAR, BLACK),
            new Tile(ELEPHANT, BLACK)
        ]);
        ...
        this.tiles.push([
            new Tile(ELEPHANT, WHITE),
            new Tile(CAR, WHITE),
            new Tile(HORSE, WHITE),
            new Tile(GUARD, WHITE),
            new Tile(CANNON, WHITE),
            new Tile(HORSE, WHITE),
            new Tile(CAR, WHITE),
            new Tile(ELEPHANT, WHITE)
        ]);
